...
selfClosing =
  P.between (P.char '<') (P.pMap2 (\_ _ -> ()) (P.char '/') (P.char '>')) $
    P.pMap2 (\tag _ -> (tag, [])) P.ident (P.many P.ws1)           
...
attribute = P.pMap2 (\key value -> (key, value)) P.ident (P.opt (P.between quoteParser quoteValueParser quoteParser))

quoteParser :: P.Parser String
quoteParser = P.between (P.char '"') (P.char '"') (P.many (P.satisfies (/= '"') "non-double-quote character"))

quoteValueParser :: P.Parser String
quoteValueParser = P.many (P.satisfies (/= '"') "non-double-quote character") `P.orElse` P.succeed "" 
