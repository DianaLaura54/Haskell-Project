...
printMatches matches = mapM_ printMatch matches
  where
    printMatch :: (TestableMonadIO io) => FileMatch -> io ()
    printMatch (FileMatch path html) = putStrLn $ path ++ "\n" ++ show html ++ "\n\n"
...
readContents (Args.Files files) = do
  ...
  return $ sequenceA fileContents

-- Helper function to read a single file and return its contents with the file path.
readSingleFile :: (TestableMonadIO io) => FilePath -> io (Result MainError (FilePath, String))
readSingleFile file = do
  ...
  if exists
    then do
      ...
      return $ Error $ FileNotFound file
...
searchFiles query files = concatMap (\(path, doc) -> map (FileMatch path) (search query doc)) files
